...
import 'package:lec1v1/sports/firebasehelper.dart';
...
class _SportsScreenFirebaseState extends State<SportsScreenFirebase> {
  ...
  void initState() {
    ...
    FirebaseHelper.getInstance().then((value) {
      _firebaseHelper = value;
      _firebaseHelper.getAllData().then((value) {
              ...
              actions: [
                ...
                ElevatedButton(
                    onPressed: () {
                      ...
                      if (date != "" && description != "") {
                        _firebaseHelper.existingCounter().then((key) {
  ...
  List<Widget> populateAlltheData() {
    ...
    filteredSportsGames.forEach((element) {
      listTiles.add(Dismissible(
        key: UniqueKey(),
        onDismissed: (_) {
          _firebaseHelper.removeSportsEntry(element.id).then((value) {
            _firebaseHelper.getAllData().then((value) {
              allSportsGames = value;
              filteredSportsGames = List.from(allSportsGames);
              controllerSearch.text = "";
